...
public class TolerancePolicyTests
{
  ...
  public void BodyMapping_Validation() {
    ...
    foreach (var el in doc.EnumerateArray()) {
      ...
      if (testpo == 3 && naif == 399)
        hasEarth = true;
      if (testpo == 10 && naif == 301)
        hasMoon = true;
    }
  ...
  public void NoToleranceLiteralsOutsidePolicy() {
    ...
    foreach (var file in Directory.GetFiles(root, "*.cs", SearchOption.AllDirectories)) {
      ...
      if (name.Equals("TolerancePolicy.cs", StringComparison.OrdinalIgnoreCase))
        continue;
      if (file.Contains(Path.Combine("bin", String.Empty)) || file.Contains(Path.Combine("obj", String.Empty)))
        continue;
