...
//   - Docs/SsdCatalog/SSDCatalog.md (HTML collapsible tree incl. optional TestPo section)
...
// Build hierarchy for HTML rendering of BSPs
...
if (includeTestPo)
{
  ...
  foreach (var t in testPoEntries.OrderBy(t => int.Parse(t.EphemerisNumber)))
  {
    ...
    md.Append("  <div class=\"indent1\"><a href=\"").Append(t.Url).Append("\">")
...
{ var dirUrl = GetDirectoryUrl(dir); var openAttr = open ? " open" : string.Empty; var indentClass = depth == 0 ? string.Empty : $" class=\"indent{depth}\""; sb.Append("<details").Append(openAttr).AppendLine(">"); sb.Append("  <summary").Append(indentClass).Append("><strong><a href=\"").Append(dirUrl).Append("\">").Append(dir.Name).Append("/</a></strong></summary>").AppendLine(); foreach (var sub in dir.Directories.Values.OrderBy(d => d.Name, StringComparer.OrdinalIgnoreCase)) RenderDirectory(sub, sb, depth + 1, open:false); foreach (var file in dir.Files.OrderBy(f => f.Name, StringComparer.OrdinalIgnoreCase)) { var ts = file.LastModified?.ToString("yyyy-MM-dd HH:mm") ?? "?"; sb.Append("  <div class=\"indent").Append(depth + 1).Append("\"><a href=\"").Append(file.Url).Append("\">").Append(file.Name).Append("</a> (").Append(file.SizeDisplay).Append(' ').Append(ts).AppendLine(")</div>"); } sb.AppendLine("</details>"); }
...
sealed class DirNode { public string Name { get; } public DirNode? Parent { get; } public Dictionary<string, DirNode> Directories { get; } = new(StringComparer.OrdinalIgnoreCase); public List<CatalogEntry> Files { get; } = new(); public DirNode(string name, DirNode? parent) { Name = name; Parent = parent; } public void AddFile(string[] parts, int index, CatalogEntry entry) { if (index == parts.Length - 1) { Files.Add(entry); return; } var dirName = parts[index]; if (!Directories.TryGetValue(dirName, out var child)) { child = new DirNode(dirName, this); Directories.Add(dirName, child); } child.AddFile(parts, index + 1, entry); } public int GetMaxDepth(int depth = 0) { var max = depth; foreach (var d in Directories.Values) { var sub = d.GetMaxDepth(depth + 1); if (sub > max) max = sub; } return max; } }
