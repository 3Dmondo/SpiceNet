...
internal static class LskParser
{
  ...
  internal static LskKernel Parse(Stream stream) {
    ...
    for (int i = 0; i < lines.Count; i++) {
      ...
      if (IsSkippable(line))
        continue;
      if (!line.StartsWith("DELTET/DELTA_AT", StringComparison.OrdinalIgnoreCase))
        continue;

      int eq = line.IndexOf('=');
      if (eq < 0)
        throw new InvalidDataException("Missing '=' in DELTET/DELTA_AT line");
      ...
      while (!blockBuilder.ToString().Contains(')')) {
        i++;
        if (i >= lines.Count)
          throw new InvalidDataException("Unterminated DELTET/DELTA_AT block");
        var l = lines[i].Trim();
        if (IsSkippable(l))
          continue;
      ...
      for (int t = 0; t < rawTokens.Length; t++) {
        ...
        t++;
        if (t >= rawTokens.Length)
          throw new InvalidDataException("Missing date token after offset");
        var dateToken = rawTokens[t];
        if (!dateToken.StartsWith('@'))
          throw new InvalidDataException($"Date token must start with '@' (got '{dateToken}')");
  ...
  static DateTimeOffset ParseDate(string token) {
    ...
    if (parts.Length != 3)
      throw new InvalidDataException($"Invalid date format '{token}'");
    if (!int.TryParse(parts[0], out var year))
      throw new InvalidDataException($"Invalid year in '{token}'");
    if (!MonthMap.TryGetValue(parts[1], out var month))
      throw new InvalidDataException($"Invalid month in '{token}'");
    if (!int.TryParse(parts[2], out var day))
      throw new InvalidDataException($"Invalid day in '{token}'");
